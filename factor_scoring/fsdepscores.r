...
W_nomiss = solve(fullresid) %*% loadings %*% solve( t(loadings) %*% solve(fullresid) %*% loadings )
...
for(i in 1:length(mp$case.idx)){
	...
	rank = rankMatrix(lambda)[1]
	
	if(rank==dim(lambda)[2]){	

	W = solve(mp_resid) %*% lambda %*% solve( t(lambda) %*% solve(mp_resid) %*% lambda )
...
saveRDS(FS, "FS_newcode_full_semifinal.Rds")
write.csv(FS,"FS_newcode_full_semifinal.csv",row.names=TRUE,quote=FALSE)
saveRDS(regression_weights,"WLSweight_newcode_full_semifinal.Rds")
write.csv(regression_weights,"WLSweight_newcode_full_semifinal.csv",row.names=TRUE,quote=FALSE)
saveRDS(miss_nomiss_cor,"MissCor_newcode_full_semifinal.Rds")
write.csv(miss_nomiss_cor,"MissCor_newcode_full_semifinal.csv",row.names=TRUE,quote=FALSE)
